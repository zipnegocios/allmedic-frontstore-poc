...
| **Corporate** | `corporateSets`, `setBlocks`, `setBlockOptions`, `setRecommendedItems`, `setColorCombos`, `setColorComboItems`, `businessRules`, `corporateAccounts`, `corporateCarts`, `quotes`, `quoteItems`, `quoteDocuments`, `taxPresets`, `validityPresets`, `quoteNumberCounters` | `corporate.ts` |
...
- Supports: `MIN_QUANTITY`, `MULTIPLES_ONLY`, `QUANTITY_RANGE`, `SIZE_MODE`, `PRICE_VISIBILITY`, `VOLUME_SCALE`, `PROMO`, `COLOR_RESTRICTION`, `VOLUME_DISCOUNT_RETAIL`, `COLOR_PAIRING`. `INVENTORY_MODE` fue eliminado del motor (`0004_supresion_stock_inventario.sql`) — la disponibilidad se maneja hoy vía `product_variants.status` (`AVAILABLE|BACKORDER|OUT_OF_STOCK`), fuera del motor de reglas.

**`corporateSets`** — Corporate set (bundle) master, modelo de **bloques de alternancia**
- `id`, `name`, `slug` (unique), `description`, `brandId` (derivada de las piezas, sin selector propio), `colorMode` (`PAIRED`|`MIXED`), `isActive`, `isFeatured`, `sortOrder`
- Composición: exactamente **2 bloques fijos** (`setBlocks`, `blockCode` `A`/`B`, `UNIQUE(set_id, block_code)`), cada uno con exactamente **2 opciones alternativas** (`setBlockOptions`) — el cliente final elige 1 opción por bloque en la ficha pública. `quantityPerSet` vive en `setBlocks` (compartido por sus 2 opciones), no por opción individual. Sin `set_items`: esa tabla fue reemplazada por este modelo (`docs/superpowers/plans/2026-07-23-ensamblador-sets-bloques-alternancia.md`).
- `setRecommendedItems` — lista libre de piezas sugeridas (misma colección/marca), sin cantidad propia ni relación con los bloques; el cliente las agrega a su cotización de forma independiente (color/talla/cantidad propios), fuera del alcance de `MIN_QUANTITY`/`COLOR_RESTRICTION`/`COLOR_PAIRING`.
- Color: `colorMode: 'PAIRED'` (por defecto) = la pieza elegida de cada bloque comparte un único color (dispara `COLOR_PAIRING` automática, ver `syncColorPairingRule`); `colorMode: 'MIXED'` = el admin cura combinaciones fijas de color por pieza (`setColorCombos`/`setColorComboItems`), el comprador solo elige entre esas combinaciones.
- Precio híbrido: `priceManual`, `priceManualSale`, `manualDiscountEnd` — todos nullable; `priceManual: null` (comportamiento por defecto) significa "precio automático": **mínimo de cada bloque × su `quantityPerSet`, sumado entre los 2 bloques** ("Desde $X"), calculado en `src/lib/set-pricing.ts` + `corporate-data-service.ts`. Un `priceManual` no nulo reemplaza esa suma en los 3 puntos de resolución de precio del servidor (`getActiveCorporateSets`, `getCorporateSetBySlug`, `getSetPricesByIds`) — el motor de pricing puro (`computeCartPricing`) nunca sabe cuál de los dos orígenes produjo el valor, solo recibe el precio ya resuelto.
- Admin panel del set (`SetForm.tsx`, `/admin/sets/[id]`) es un "ensamblador": embebe `ProductForm` (crear/editar una pieza sin salir del set, drawer a pantalla casi completa) y `RuleForm` (crear/editar reglas de ámbito Set del set actual) — ambos componentes aceptan `embedded`/`onSaved`/`onCancel` para funcionar tanto en su página propia como en un drawer. `BlockSection.tsx` (× 2) reemplaza al antiguo `PiecesSection.tsx`; `RecommendedItemsSection.tsx` gestiona la lista libre.
...
5. **Drizzle schema — corporate catalog** — `corporate.ts` (corporate_sets, set_blocks, set_block_options, set_recommended_items, set_color_combos, set_color_combo_items, business_rules, corporate_accounts, corporate_carts, quotes, quote_items, quote_documents, tax_presets, validity_presets, quote_number_counters) is implemented and exported from `src/db/schema/index.ts`. `set_items` (piezas planas) fue reemplazada por el modelo de bloques de alternancia — ver `docs/superpowers/plans/2026-07-23-ensamblador-sets-bloques-alternancia.md`. See `.claude/pre-plans/PLAN-catalogos-segmentados.md` for the original rules-engine design.
