...
const UpdateProductSchema = z.object({
  ...
  // `.trim()` + charset — ver comentario equivalente en `products/route.ts` (evita que
  // un espacio colado persista como parte del código y rompa la comparación de unicidad).
  code: z.string().trim().min(1).regex(/^[A-Z0-9_-]+$/, 'Código de estilo con formato inválido').optional(),
